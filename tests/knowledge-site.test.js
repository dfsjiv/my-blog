...
assert.match(html, /class="knowledge-panel knowledge-home-search"[\s\S]*id="knowledgeSideSearch"[\s\S]*class="knowledge-home-classification"/);
assert.doesNotMatch(html, /class="knowledge-sidebar knowledge-right-sidebar"/);
assert.doesNotMatch(html, /id="knowledgeTopicList"|id="knowledgeTagCloud"|id="knowledgeStats"/);
assert.doesNotMatch(html, /sidebar-miku-cute-transparent-v2\.png/);
...
assert.doesNotMatch(html, /data-knowledge-nav-menu="links"/);
assert.match(html, /data-knowledge-nav-menu="language"/);
assert.match(html, /data-knowledge-language="en"[\s\S]*>英语</);
assert.match(html, /data-knowledge-language="zh"[\s\S]*>中文</);
assert.match(html, /class="knowledge-legacy-nav-controls" hidden[\s\S]*id="knowledgeSearchButton"[\s\S]*id="knowledgeLanguageButton"/);
assert.match(i18n, /'语言': 'Language'/);
assert.match(i18n, /'英语': 'English'/);
assert.match(i18n, /'中文': 'Chinese'/);
...
assert.doesNotMatch(html, /id="knowledgeLinkMenu"/);
assert.match(html, /id="knowledgeAboutMenu" role="menu" hidden/);
assert.match(html, /data-social-link="bilibili"[\s\S]*target="_blank" rel="noopener noreferrer"/);
assert.match(html, /data-social-link="github"[\s\S]*target="_blank" rel="noopener noreferrer"/);
assert.match(html, /data-social-link="zhihu"[\s\S]*target="_blank" rel="noopener noreferrer"/);
assert.match(html, /data-social-link="nowcoder"[\s\S]*target="_blank" rel="noopener noreferrer"/);
...
assert.match(css, /\.knowledge-layout\s*\{[\s\S]*grid-template-columns:\s*minmax\(0, 1fr\)/);
...
assert.match(site, /function isAdmin\(\)/);
...
assert.match(site, /shell\.querySelectorAll\('\[data-social-link\]'\)/);
...
assert.match(html, /knowledge-site-v2\.css\?v=20260926-backgrounds-1/);
assert.match(css, /\.knowledge-hero-waves\s*\{[\s\S]*height:\s*220px/);
assert.match(css, /viewBox='0 0 1440 220'/);
...
assert.match(html, /data-knowledge-route="profile">个人中心/);
assert.match(site, /accountRole\.textContent/);
assert.match(site, /route === 'profile'[^\n]*renderProfile/);
assert.match(site, /function renderProfile\(\)/);
...
assert.match(html, /id="knowledgeHomeClassificationTitle">文章分类/);
assert.match(html, /data-home-latest-sort="latest"/);
assert.match(html, /data-home-latest-sort="updated"/);
assert.match(html, /data-home-latest-sort="oldest"/);
assert.match(html, /data-home-latest-type="article"/);
assert.match(html, /data-home-latest-type="solution"/);
assert.match(html, /data-home-latest-type="note"/);
assert.match(html, /data-home-latest-type="project"/);
assert.match(html, /data-home-latest-type="essay"/);
assert.match(site, /function homeLatestFilters\(page\)/);
assert.match(site, /function reloadHomeLatest\(\)/);
assert.match(site, /homeLatestSort:\s*'latest'/);
assert.match(site, /homeLatestType:\s*''/);
assert.match(i18n, /'文章分类': 'Post Categories'/);
...
assert.match(html, /data-knowledge-route="backgrounds"/);
assert.match(site, /repository\.getBackgrounds\(\)/);
assert.match(site, /function renderBackgroundManager/);
assert.match(site, /repository\.uploadBackground/);
assert.match(site, /repository\.updateBackground/);
assert.match(site, /repository\.deleteBackground/);
assert.match(css, /\.knowledge-background-upload/);
...
assert.match(site, /role === 'admin' \|\| role === 'user'/);
assert.match(site, /if \(!isAdmin\(\)\) return navigate\('home'/);
assert.match(writer, /普通用户正文上限 256 KB/);
assert.match(writer, /普通用户上传的图片不能超过 2 MB/);
