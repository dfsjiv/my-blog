...
assert.match(html, /data-icon-zh="技" data-icon-en="T"/);
assert.match(html, /data-icon-zh="游" data-icon-en="GM"/);
assert.doesNotMatch(html, /<small>内容频道<\/small>/);
...
assert.match(site, /post\.source === 'legacy-blog' \? '转换并编辑' : '编辑'/);
assert.match(site, /repository\.createEditableLegacyPost/);
...
assert.match(editorAdapter, /function looksLikeSourceCode/);
assert.match(editorAdapter, /function detectCodeLanguage/);
assert.match(editorAdapter, /insertCodeBlock\(editorRef, plain, detectCodeLanguage\(plain\)\)/);
assert.match(editorAdapter, /type:\s*'codeBlock'/);
assert.match(editorAdapter, /function selectionToCodeBlock/);
assert.match(editorAdapter, /function selectionToText/);
assert.match(writer, /selection-code/);
assert.match(writer, /selection-text/);
assert.match(writer, /dataset\.writerMode/);
assert.match(writer, /Markdown 分屏模式/);
assert.match(writer, /knowledge-writer-source-input/);
assert.match(writer, /function currentMarkdown\(\)/);
assert.match(writer, /markdownRenderer\.render\(sourceInput\.value, sourcePreviewBody\)/);
assert.match(writer, /contentMarkdown:\s*currentMarkdown\(\)/);
assert.match(writer, /repository\.uploadImage[\s\S]*insertSourceText/);
assert.match(writer, /adapter\.selectionToCodeBlock/);
assert.match(writer, /adapter\.selectionToText/);
assert.match(writer, /event\.target === bubbleModeSelect[\s\S]*adapter\.selectionToText/);
...
assert.match(html, /data-knowledge-nav-menu="links"[\s\S]*?链接 <span aria-hidden="true">⌄<\/span>/);
...
assert.match(site, /main\.style\.visibility = 'hidden';[\s\S]*const hydrationPromise = updateRoute\(\)/);
assert.doesNotMatch(site, /await updateRoute\(\)/);
assert.match(site, /hydrationPromise\.catch/);
assert.match(site, /正在加载文章信息/);
assert.match(site, /正在生成目录/);
assert.match(site, /nextPage\.style\.removeProperty\('visibility'\)/);
assert.match(site, /event\.target !== node/);
assert.match(site, /shell\.appendChild\(previousPage\)/);
assert.match(site, /shell\.appendChild\(nextPage\)/);
assert.match(site, /previousPage\.dataset\.snapshotRoute = previousRoute/);
assert.match(site, /nextPage\.dataset\.snapshotRoute = shell\.dataset\.route \|\| state\.route/);
assert.match(site, /const nextRect = main\.getBoundingClientRect\(\)/);
assert.match(site, /width: nextRect\.width \+ 'px'/);
assert.match(site, /function resetRouteScroll\(\)/);
assert.match(site, /await waitForRouteLayout\(\)/);
assert.match(site, /shell\.classList\.add\('knowledge-route-transitioning'\)/);
assert.match(site, /shell\.classList\.remove\('knowledge-route-transitioning'\)/);
...
assert.match(css, /\.knowledge-route-snapshot\[data-snapshot-route="games"\] \.knowledge-route-view/);
assert.match(css, /\.knowledge-site\.knowledge-route-transitioning[\s\S]*?overflow-anchor:\s*none/);
assert.match(css, /translate3d\(-100%, 0, 0\)/);
assert.match(css, /translate3d\(100%, 0, 0\)/);
assert.match(css, /@keyframes knowledge-page-slide-out/);
assert.match(css, /@keyframes knowledge-page-slide-in/);
